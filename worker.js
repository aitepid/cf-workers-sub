...
///////////////////////////////////////////////////////安全与错误处理增强///////////////////////////////////////////////
// 登录失败次数追踪 [IP -> {failures, firstFailureTime}]
const 登录失败记录 = new Map();
const 登录最大失败次数 = 5;
const 登录锁定时间毫秒 = 30 * 60 * 1000; // 30分钟
const 错误日志上限 = 100;
const 错误日志缓冲区 = [];

/**
 * 集中式错误日志记录
 * @param {string} 模块 - 错误来源模块
 * @param {string|Error} 错误 - 错误信息或 Error 对象
 * @param {Object} 额外信息 - 额外的上下文数据
 */
function 记录错误(模块, 错误, 额外信息 = {}) {
	const 消息 = 错误 instanceof Error ? 错误.message : String(错误);
	const 堆栈 = 错误 instanceof Error ? 错误.stack : null;
	const 日志条目 = { 模块, 消息, 堆栈, 时间: Date.now(), ...额外信息 };
	
	// 缓冲区存储，避免 KV 写入开销
	if (错误日志缓冲区.length < 错误日志上限) {
		错误日志缓冲区.push(日志条目);
	}
	
	if (调试日志打印) {
		console.error(`[${模块}] ${消息}${堆栈 ? '\n' + 堆栈 : ''}`);
	}
}
...
function 检查登录限制(IP) {
	...
	const 记录 = 登录失败记录.get(IP);
	if (!记录) return { allowed: true };
	
	const 失败次数 = 记录.failures;
	if (失败次数 < 登录最大失败次数) return { allowed: true };
	
	const 锁定过期时间 = 记录.firstFailureTime + 登录锁定时间毫秒;
	const 剩余时间 = 锁定过期时间 - Date.now();
	...
	return { allowed: false, remainingTime: Math.ceil(剩余时间 / 1000) };
}
...
function 记录登录失败(IP) {
	...
	let 记录 = 登录失败记录.get(IP);
	if (!记录) {
		记录 = { failures: 0, firstFailureTime: Date.now() };
		登录失败记录.set(IP, 记录);
	}
	...
	// 清理过期的记录
	if (记录.failures >= 登录最大失败次数) {
		const 锁定过期时间 = 记录.firstFailureTime + 登录锁定时间毫秒;
		if (锁定过期时间 - Date.now() <= 0) {
			登录失败记录.delete(IP);
		}
	}
}
 ...
 * @param {string} IP - 客户端 IP
 */
function 重置登录失败记录(IP) {
	if (IP) 登录失败记录.delete(IP);
}
 ...
 * @returns {string|null} 验证通过的字符串或 null
 */
function 验证输入(值, 选项 = {}) {
	const { 
		最大长度 = 2048, 
	...
	if (值 === undefined || 值 === null) return 默认值;
	
	const 字符串值 = String(值);
	const  trimmed = 允许空白 ? 字符串值 : 字符串值.trim();
	
	if (trimmed.length > 最大长度) return null;
	if (trimmed.length < 最小长度) return null;
	if (正则表达式 && !正则表达式.test(trimmed)) return null;
...
const Pages静态页面 = 'https://aitepid.github.io/cf-workers-sub';
///////////////////////////////////////////////////////全局常量和工具函数///////////////////////////////////////////////
...
export default {
	async fetch(request, env, ctx) {
		...
		if (访问路径 === 'version') {// 版本信息接口
			const 请求UUID = 验证输入(url.searchParams.get('uuid'), { 最大长度: 64 });
			...
			if (env.KV && typeof env.KV.get === 'function') {
				...
				if (区分大小写访问路径 === 加密秘钥 && 加密秘钥 !== '勿动此默认密钥，有需求请自行通过添加变量KEY进行修改') {//快速订阅
					const 密钥 = 验证输入(加密秘钥, { 最大长度: 128 });
					if (!密钥) { 记录错误('Access', '秘钥验证失败'); return new Response('Forbidden', { status: 403 }); }
					...
					if (request.method === 'POST') {
						...
						const 输入密码 = 验证输入(params.get('password'), { 最大长度: 128, 允许空白: true });
						// 检查登录速率限制
						const 访问IP = request.headers.get('CF-Connecting-IP') || request.headers.get('True-Client-IP') || 'unknown';
						const 限制检查结果 = 检查登录限制(访问IP);
						if (!限制检查结果.allowed) {
							const 响应 = new Response(JSON.stringify({ success: false, error: `登录失败次数过多，请 ${限制检查结果.remainingTime} 秒后重试` }), { status: 429, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
							记录错误('Login', `登录被速率限制: ${访问IP}`);
							return 响应;
						}
						if (输入密码 === (typeof 管理员密码 === 'string' ? 管理员密码.replace(/[\r\n]/g, '') : 管理员密码)) {
							// 密码正确，重置失败记录并设置cookie
							重置登录失败记录(访问IP);
					...
					if (访问路径 === 'admin/log.json') {// 读取日志内容
						...
						try {
							const Email = 验证输入(url.searchParams.get('Email'), { 最大长度: 254, 正则表达式: /^[^\s@]+@[^\s@]+\.[^\s@]+$/ });
							const GlobalAPIKey = 验证输入(url.searchParams.get('GlobalAPIKey'), { 最大长度: 256 });
							const AccountID = 验证输入(url.searchParams.get('AccountID'), { 最大长度: 64 });
							const APIToken = 验证输入(url.searchParams.get('APIToken'), { 最大长度: 256 });
							if (!Email && !AccountID) return new Response(JSON.stringify({ error: '缺少邮箱或AccountID参数' }), { status: 400, headers: { 'Content-Type': 'application/json;charset=utf-8' } });
						...
						const ADD_API_URL = 验证输入(url.searchParams.get('url'), { 最大长度: 2048 });
						const ADD_API_PORT = 验证输入(url.searchParams.get('port'), { 最大长度: 5, 正则表达式: /^\d{1,5}$/ });
						if (ADD_API_URL) {
							try {
								new URL(ADD_API_URL);
								const 请求优选API内容 = await 请求优选API([ADD_API_URL], ADD_API_PORT || '443');
						...
						const 代理参数 = 验证输入(url.searchParams.get(代理协议), { 最大长度: 2048 });
...
const DoH缓存 = {};
const DoH缓存最大条目 = 256;
const DoH缓存清理间隔毫秒 = 5 * 60 * 1000; // 5分钟清理一次
let DoH上次清理时间 = 0;

// 订阅转换缓存
const 订阅转换缓存 = {};
const 订阅转换缓存最大条目 = 64;
const 订阅缓存过期毫秒 = 5 * 60 * 1000; // 5分钟
...
async function DoH查询(域名, 记录类型, DoH解析服务 = "https://cloudflare-dns.com/dns-query") {
	...
	try {
		...
		// DoH 缓存定期清理
		const 当前清理时间 = Date.now();
		if (当前清理时间 - DoH上次清理时间 > DoH缓存清理间隔毫秒) {
			DoH上次清理时间 = 当前清理时间;
			const 过期键 = Object.keys(DoH缓存).filter(k => 当前清理时间 >= DoH缓存[k].过期时间);
			for (const key of 过期键) delete DoH缓存[key];
			log(`[DoH缓存] 清理了 ${过期键.length} 条过期缓存`);
		}
		if (Object.keys(DoH缓存).length >= DoH缓存最大条目) {
			const 最早键 = Object.keys(DoH缓存).sort((a, b) => DoH缓存[a].过期时间 - DoH缓存[b].过期时间)[0];
			delete DoH缓存[最早键];
			log(`[DoH缓存] 达到上限(${DoH缓存最大条目})，清理最旧条目`);
		}
...
/**
 * 清理过期的订阅转换缓存
 * @param {number} 当前时间 - 当前时间戳
 */
function 清理订阅转换缓存(当前时间 = Date.now()) {
	const 过期键 = Object.keys(订阅转换缓存).filter(k => 当前时间 >= 订阅转换缓存[k].过期时间);
	for (const key of 过期键) {
		delete 订阅转换缓存[key];
	}
	if (过期键.length > 0) log(`[订阅缓存] 清理了 ${过期键.length} 条过期缓存`);
}
